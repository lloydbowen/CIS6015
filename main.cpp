...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods) 
{
	...
	switch (key)
	{
		...
		case GLFW_KEY_1:
			...
			//below also works but you need to code diff keys to go back and forth
			/*if (cCamera* pCamera = &Scene.m_Camera[0])
				pCamera = Scene.m_Camera += 1;*/
		break;
		...
		//changed zoom in/out value from 0.25 to 1 so i can zoom in faster
		case GLFW_KEY_UP:
		{
			// zoom out along z-axis
			if (pCamera->m_pos.z > 2.0f)
				pCamera->m_pos.z -= 1.0f;
		}break;
